...
- Empty-prompt hints resolve once in `promptPlaceholder` (input-utils.ts) and both clients render them: Ink through `PromptInput.placeholder`, the ANSI renderer dim next to its block cursor (truncated, never wrapped, so a hint cannot add a prompt row). `RenderLine.prefixBold` keeps the prompt pointer bold in both; the ANSI busy marker is the running-colored `⟳`.
