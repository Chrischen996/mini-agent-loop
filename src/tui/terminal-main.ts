...
import { writeAgentMd } from "../init-agent-md.ts";
import { resolveAgentMdContent } from "../init-agent.ts";
...
async function submitInput(
  ...
  if (store.getState().busy && /^(?:resume|\/(?:clear|resume|model|profiles?|plan(?:-|\s|$)|init))/i.test(text)) {
  ...
  if (initCommand) {
    ...
    try {
      store.dispatch({
        type: "SET_STATUS",
        status: initCommand.template ? "Preparing AGENT.MD template…" : "Analyzing project for AGENT.MD…",
      });
      const resolution = await resolveAgentMdContent({
        cwd: deps.cwd,
        template: initCommand.template,
        onEvent: (event) => {
          if (event.type === "tool_start") {
            store.dispatch({ type: "SET_STATUS", status: `Analyzing project: ${event.call.name}` });
          }
        },
      });
      if (initCommand.print) {
        store.dispatch({ type: "ADD_NOTICE", title: "AGENT.MD Preview", text: resolution.content });
        return;
      }
      const written = await writeAgentMd({ cwd: deps.cwd, content: resolution.content, force: initCommand.force });
      ...
      const origin = resolution.source === "llm"
        ? ` Generated by ${resolution.model} in ${resolution.turns} turn${resolution.turns === 1 ? "" : "s"}.`
        : " Created from the offline template.";
      const warning = resolution.warning ? ` ${resolution.warning}` : "";
      store.dispatch({
        type: "ADD_NOTICE",
        title: "AGENT.MD",
        text: `${written.overwritten ? "Overwrote" : "Created"} AGENT.MD.${origin}${warning} Instructions are now active for subsequent turns.`,
      });
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error);
      store.dispatch({ type: "ADD_NOTICE", title: "Init Failed", text: detail });
    } finally {
      store.dispatch({ type: "SET_STATUS", status: "Ready" });
    }
    return;
  }
