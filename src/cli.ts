...
export type ParsedCliArgs = {
  ...
  init?: { force: boolean; print: boolean; template: boolean };
};
...
export function parseCliArgs(argv: string[]): ParsedCliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "init") {
      init = { force: false, print: false, template: false };
      continue;
    }
    if (arg === "--init-force") {
      if (!init) init = { force: false, print: false, template: false };
      init.force = true;
      continue;
    }
    if (arg === "--init-print") {
    ...
    const resolution = await resolveAgentMdContent({
      ...
      onEvent: (event) => {
        if (event.type === "tool_start") {
          ...
          process.stderr.write(`[init] ${event.call.name} ${args.length > 120 ? `${args.slice(0, 120)}…` : args}\n`);
        }
      },
    });
    if (resolution.warning) {
      process.stderr.write(`[init] ${resolution.warning}\n`);
    }
    if (options.print) {
      process.stdout.write(`${resolution.content}`);
      return 0;
    }
    const written = await writeAgentMd({ cwd: options.cwd, content: resolution.content, force: options.force });
    const origin = resolution.source === "llm"
      ? ` (generated by ${resolution.model} in ${resolution.turns} turn${resolution.turns === 1 ? "" : "s"})`
      : "";
    process.stdout.write(`${written.overwritten ? "Overwrote" : "Created"} ${written.path}${origin}\n`);
    return 0;
  } catch (error) {
