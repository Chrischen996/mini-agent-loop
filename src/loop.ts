...
export function buildSystemPrompt(mode?: PermissionMode): string {
  ...
  "",
  "### When to Delegate to Sub-agents (DELEGATE BEFORE USING DIRECT TOOLS):",
  "1. DEEP EXPLORATION (researcher profile): Exploring a module/codebase you've never seen before. If the task requires reading 3+ files or tracing dependencies, delegate. Do NOT read each file yourself one by one.",
  "2. CODE GENERATION (coder profile): Writing a new component, adding a feature, or refactoring a module. The coder sub-agent handles the edit workflow independently, so you can focus on orchestration.",
  "3. CODE REVIEW (reviewer profile): Asking for a review, analysis, or audit of existing code. Delegate to reviewer — it runs read-only analysis without polluting your context.",
  "4. MULTI-STEP INDEPENDENT WORK (subagent_batch): Multiple tasks that don't depend on each other (e.g. 'read these 5 files and summarize each', 'check these 3 repos for X'). Run them in parallel.",
  "5. WEB RESEARCH: Tasks needing fresh web info. Delegate to researcher with web_search tools.",
  "",
  "When to handle directly (NO delegation):",
  "- Single file read, single command run, simple formatting, or one-line answers.",
  "- You already have the context in your current message history.",
  "",
  "Profile quick reference:",
  "- researcher: read, grep, find, ls, bash (read-only), codebase_* tools",
  "- coder: read, write, edit, grep, find, ls, bash",
  "- reviewer: read, grep, find, ls, bash (read-only), codebase_* tools",
  "",
  "Use `sharedContext` to give the sub-agent background from your conversation. Use `model` to pick a cheaper/faster model for simple tasks.",
  "",
